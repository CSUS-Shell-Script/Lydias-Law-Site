...
# Test the search bar functionality for admin view of transaction history
class AdminTransactionsSearchTests(TestCase):
    ...
    def setUp(self):
        ...
        self.invoice2 = Invoice.objects.create(
            ...
            hosted_invoice_url='https://stripe.com/inv/456'
        )
    ...
    def test_search_by_first_name(self):
        ...
        # Make the request
        response = self.client.get('/administrator/transactions/', {'search': 'John'})
        
        # Check response
        self.assertEqual(response.status_code, 200)
        self.assertEqual(len(response.context['page_obj']), 1)
        self.assertEqual(response.context['page_obj'][0].user.first_name, 'John')
    
    # Search by last name
    def test_search_by_last_name(self):
        self.client.login(email='admin@example.com', password='adminpass123')
        response = self.client.get('/administrator/transactions/', {'search': 'Smith'})
        
        self.assertEqual(response.status_code, 200)
        self.assertEqual(len(response.context['page_obj']), 1)
        self.assertEqual(response.context['page_obj'][0].user.last_name, 'Smith')
    
    # Search by email
    def test_search_by_email(self):
        self.client.login(email='admin@example.com', password='adminpass123')
        response = self.client.get('/administrator/transactions/', {'search': 'bob@example.com'})
        
        self.assertEqual(response.status_code, 200)
        self.assertEqual(len(response.context['page_obj']), 1)
        self.assertEqual(response.context['page_obj'][0].user.email, 'bob@example.com')
    
    # No maching results
    def test_search_no_results(self):
        self.client.login(email='admin@example.com', password='adminpass123')
        response = self.client.get('/administrator/transactions/', {'search': 'xyzabc123'})
    ...
    def test_search_with_status_filter(self):
        self.client.login(email='admin@example.com', password='adminpass123')
        response = self.client.get('/administrator/transactions/', {
            'search': 'John',
            'status': 'paid'
        })
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(len(response.context['page_obj']), 1)
        self.assertEqual(response.context['page_obj'][0].status, 'PAID')
    
    # Combine search with amount filter
    def test_search_with_amount_filter(self):
        self.client.login(email='admin@example.com', password='adminpass123')
        response = self.client.get('/administrator/transactions/', {
            'search': 'John',
            'min_amount': '50',
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(len(response.context['page_obj']), 1)
        self.assertEqual(response.context['page_obj'][0].amount_dollars, 100.00)
    
    # Empty search returns all invoices
    def test_search_empty_query(self):
        self.client.login(email='admin@example.com', password='adminpass123')
        response = self.client.get('/administrator/transactions/', {'search': ''})
        
        self.assertEqual(response.status_code, 200)
        # Should return all 3 invoices
        self.assertEqual(len(response.context['page_obj']), 3)
    
    # Search works with pagination
    def test_search_pagination(self):
        # Create extra invoices for John to test pagination
        for i in range(25):
            Invoice.objects.create(
                user=self.client1,
                amount=10000 + i,
                status='PAID',
                stripe_invoice_id=f'inv_test_{i}'
            )
    ...
    # Search and filters are preserved regardless of page
    def test_search_preserves_filters_are_preserved(self):
        # Create many invoices for John
        for i in range(25):
            Invoice.objects.create(
                user=self.client1,
                amount=10000 + i,
        ...
        response1 = self.client.get('/administrator/transactions/', {
            'search': 'John',
            'per_page': '20'
        })
        
        # Then go to page 2
        response2 = self.client.get('/administrator/transactions/', {
            'page': '2',
            'search': 'John',
            'per_page': '20'
        })
        ...
        self.assertEqual(response2.status_code, 200)
        # Page 2 should have fewer items
        self.assertTrue(len(response2.context['page_obj']) <= 20)
